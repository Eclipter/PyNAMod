...
class Iterator:
    ...
    def run(self,movable_steps,target_accepted_steps=int(1e5),max_steps=int(1e6),transfer_to_memory_every=None,save_every=1,
        ...
        print("Start time:",time.strftime('%D %H:%M:%S',prev_time))
        try:
            while self.total_step < max_steps and self.accepted_steps < target_accepted_steps:
                ...
                if mute:
                    ...
                    if (cur_time.tm_min - prev_time.tm_min) == 5:
                        prev_time = cur_time
                        print(f'Accepted steps: {self.accepted_steps}, acceptance rate: {acceptance_rate}%')
                else:
    ...
    def _set_change_indices(self,movable_steps,integration_mod):
        self.movable_ind = torch.arange(self.trajectory.data_len,dtype=int)[movable_steps]
